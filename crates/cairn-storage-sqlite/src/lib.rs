//! SQLite backend for [`cairn_storage::Storage`].
//!
//! SQLite is the default because a community node operator should be able to run
//! `cairnd` with no external services at all. Postgres is for the hub and for nodes past
...
use cairn_storage::attr::{AttrKey, AttrMap};
use cairn_storage::model::{Account, Ban, BanScope, Credential, NewAccount};
use cairn_storage::{validate_account_name, Result, Storage, StorageError};
        ...
        if current > SCHEMA_VERSION {
            return Err(StorageError::Backend(format!(
                "database schema is version {current} but this build understands only \
                 {SCHEMA_VERSION}; upgrade cairnd rather than downgrading the database"
            )));
        }
...
mod tests {
    use super::*;
    use cairn_storage::conformance;
    use cairn_storage::{FlushPolicy, WriteBehind};
    ...
    fn data_survives_reopening_the_file() {
        ...
        let dir = std::env::temp_dir().join(format!("cairn-sqlite-{}", std::process::id()));
