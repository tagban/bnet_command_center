...
impl AdBanner {
    ...
    pub fn matches(&self, product: FourCc, language: Option<FourCc>) -> bool {
        let product_ok = self.product.is_none_or_eq(product);
...
/// Small helper so the match above reads as one condition.
trait NoneOrEq {
    fn is_none_or_eq(self, other: FourCc) -> bool;
}

impl NoneOrEq for Option<FourCc> {
    fn is_none_or_eq(self, other: FourCc) -> bool {
        match self {
            None => true,
            Some(v) => v == other,
...
pub fn is_random_product(product: FourCc) -> bool {
    matches!(
        product,
        p if p == cairn_proto::product::WAR3 || p == cairn_proto::product::W3XP
    )
}
